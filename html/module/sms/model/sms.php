...
//		$to = $to . $settings['adminMobile'];
		...
		if($settings[proto] == "REST"){
			...
			switch ($client->SendSms($parameters)->SendSmsResult){
				case 0:
					$system->watchDog->exception("e", $lang[error] . '0', 'اس ام اس پانل - ‫نام كاربری یا رمز عبور صحیح نمی باشد‬');
					break;
				case 1:
					foreach ($to as $t){
						$system->dbm->db->insert("`$settings[smsObject]`", "`active`, `timeStamp`, `owner`, `group`, `or`, `ow`, `ox`, `gr`, `gx`, `from`, `to`, `message`", "1, $time, 1, 1, 1, 1, 1, 1, 1, '$settings[from]', '$t', '$message'");
				...
				case 2:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '2', 'اس ام اس پانل - ‫اعتبار كافی نیست‬');
					break;
				case 3:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '3', 'اس ام اس پانل - ‫محدودیت در ارسال روزانه‬');
					break;
				case 4:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '4', 'اس ام اس پانل - ‫محدودیت در حجم ارسال‬');
					break;
				case 5:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '5', 'اس ام اس پانل - ‫شماره فرستنده معتبر نیست‬');
					break;
				case 6:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '6', 'اس ام اس پانل - ‫سامانه در حال بروز رسانی می باشد‬‬');
					break;
				case 7:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '7', 'اس ام اس پانل - ‫متن پیامك شامل كلمات فیلتر شده می باشد‬‬');
					break;
				case 8:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '8', 'اس ام اس پانل - ‫‫عدم رسیدن به حداقل ارسال‬‬');
					break;
				case 9:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '9', 'اس ام اس پانل - ‫ارسال از خطوط عمومی از طریق وب سرویس امكان پذیر نمی باشد‬‬');
					break;
				case 10:
					if($show)
					$system->watchDog->exception("e", $lang[error] . '10', 'اس ام اس پانل - ‫‫كاربر مسدود شده است‬‬');
					break;
				default:
					if($show)
