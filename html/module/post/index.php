...
switch ($sysVar[mode]){
	...
	case "v_addObject":
		$system->xorg->smarty->assign("category", $c_post->c_hierarchicalListCategory(0));
		$system->xorg->smarty->assign("authors", $system->xorg->htmlElements->selectElement->select('authors', $system->xorg->combo(array('id', 'firstName', 'lastName'), "user", "((`firstName` <> '' AND `lastName` <> '') OR (`email` <> '')) AND `gid` = 11")));
	...
	case "c_jsonList":
		$system->xorg->smarty->assign("jsonList", $c_post->c_jsonList($_GET['filter']));
		break;		
	default:
